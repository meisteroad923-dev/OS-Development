...
int main(int argc, char* argv[]){
    ...
    if ((iSourceFd = open(argv[1], O_RDWR | O_BINARY)) == -1 )
    {
    ...
    //파일 크기를 섹터 크기이 512바이트로 맞추기 위해 나머지 부분을 0x00으로 채움
    ...
    if((iSourceFd = open( argv[2], O_RDWR | O_BINARY)) == -1){
        fprintf(stderr, "[ERROR] %s open fail\n", argv[2]);
        exit(-1);
    }
...
void WriteKernelInformation(int iTargetFd, int iKernelSectorCount){
    ...
    if (lPosition == -1){
        fprintf(stderr, "lseek fail. Return valus = %d, errno = %d, %d\n", lPosition, errno, SEEK_SET);
        exit(-1);
    }
