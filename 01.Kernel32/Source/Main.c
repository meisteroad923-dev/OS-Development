...
void Main( void ){
    ...
    kPrintString(0, 4, "Minium Memory Sizse Check...................[    ]");
    if( kIsMemoryEnough() == FALSE ){
        kPrintString(45, 4, "Fail");
        kPrintString(0, 5, "Not Enough Memory~!! DIA64 OS Requires Over 64Mbyte Memory~!!");
        while(1);
    } else {
...
//IA-32e 모드용 커널 영역을 0으로 초기화 하는 함수
BOOL kInitializeKernel64Area( void ){
    DWORD* pdwCurrentAddress;

    //초지화를 시작할 어드레스인 0x100000(1MB)을 설정
...
//DIA64 OS를 실행하기에 충분한 메모리를 가지고 있는지 체크
BOOL kIsMemoryEnough( void ){
    DWORD* pwdCurrentAddress;

    //0x100000(1MB)부터 검사 시작
    pwdCurrentAddress = (DWORD*) 0x10000;
